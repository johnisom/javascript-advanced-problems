...
const makeModels = { any: options.model };
cars.forEach(({ make, model }) => {
  makeModels[make] = makeModels[make] || [];
  if (!makeModels[make].includes(model)) {
    makeModels[make].push(model);
  }
});
...
$(() => {
  ...
  $make.on('change', () => {
    $model.html(optionsTemplate({ values: makeModels[$make.val()] }));
  });
  ...
  $selects.each((_, select) => {
    $(select).html(optionsTemplate({ values: options[select.id]}));
  });
